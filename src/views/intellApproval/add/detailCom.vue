...
watch(
  ...
  (id) => {
    if (id) {
      ...
      const twelveMonthsAgo = now.subtract(11, "month");
      monthRange.value = [
        twelveMonthsAgo.startOf("month").format("YYYY-MM-DD"),
...
function changeMonth() {
  if (monthRange.value && monthRange.value[1]) {
    monthRange.value[0] = dayjs(monthRange.value[0])
      .startOf("month")
      .format("YYYY-MM-DD");
    monthRange.value[1] = dayjs(monthRange.value[1])
      .endOf("month")
      .format("YYYY-MM-DD");
  }
...
function init(id) {
  Promise.all([
    ...
    .then((results) => {
      ...
      result3?.map((item) => {
        item.count = String(item.count * 10000);
      });
      console.log("fffffffffffffffff2222222", result3);
