<template>
  <div>
    <div
      v-if="info?.result && info?.result.length < 15"
      class="risk-result"
      @click="handleResult"
    >
      {{ info?.result }}
    ...
    >
      ...
      <el-tabs type="card" v-if="sourceList.length > 1">
        <el-tab-pane :label="item.key" v-for="(item, i) in sourceList" :key="i">
          <abnormal-ty
            v-if="riskType == abnormalStatus.TY"
            :info="item"
            :columns="columns"
          ></abnormal-ty>
          <abnormal-djpl
            v-else-if="riskType == abnormalStatus.DJPL"
            :info="item"
            :columns="columns"
          ></abnormal-djpl>
          <abnormal-cfwjy
            v-else-if="riskType == abnormalStatus.CFWJY"
            :info="item"
            :columns="columns"
          ></abnormal-cfwjy>
          <abnormal-jxxsp
            v-else-if="riskType == abnormalStatus.JXXSP"
            :info="item"
          ></abnormal-jxxsp>
          <abnormal-glyc
            v-else-if="riskType == abnormalStatus.GLYC"
            :info="item"
            :columns="columns"
          ></abnormal-glyc>
          <abnormal-htzt
